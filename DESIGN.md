...
- **`src/components/Toggle.tsx`** — 트랙 + 슬라이딩 손잡이 스위치. 기본은 폼 필드가 아니라 "바로 적용되는 설정"에 쓴다(별도 저장 버튼 없이 토글 즉시 반영되는 경우). `role="switch"`로 접근성 시맨틱을 맞춘다. 마이페이지의 "자동 연장"은 두 버튼 중 하나를 고르는 `RenewalToggle`이라 이름은 비슷하지만 별개 컴포넌트/패턴. 첫 실사용은 `SeasonFeeFields`의 "정산 기능 사용" — 폼 안에 있어 최종 반영은 제출을 기다리지만, 켜고 끄는 즉시 아래 필드가 보였다 사라지는 것 자체는 여전히 "바로 적용"이라 슬라이드 스위치가 체크박스보다 잘 맞는다고 판단했다.
...
- 전부 페이지 이동이라 `<Link>`/`<a>`(로그아웃만 인증 처리가 필요해 `<button>`)이고, 클릭하면 `onClick`으로 드로어를 바로 닫는다 — 내비게이션이 끝나길 기다리지 않고 즉시 피드백을 준다.
- 로그아웃은 원래 홈 화면 헤더에 `마이페이지` 링크와 나란히 어색하게 붙어있었다 — 계정 관련 액션인데 정작 계정 화면(설정)이 아니라 홈에만 있었던 것. 햄버거 메뉴가 생기면서 다른 화면 전환 항목들과 같은 자리(전역 메뉴)로 옮기는 게 자연스러워 같이 정리했다.
...
- 사진 영역은 고정 높이 `h-[60vh]`다 — 처음엔 `flex-1 min-h-0`로 남는 공간을 채우게 했는데, 그러면 `<img>`의 `h-full`이 퍼센트 높이라 상위에 확정된 높이가 없을 때(자동 높이 flex 컬럼) 이미지의 실제 세로 비율대로 부풀어서, 세로로 긴 사진(예: 914px)이 카드 전체 높이를 밀어내고 그 초과분이 카드의 `overflow-hidden`에 잘리며 하단 리액션 바까지 같이 가려지는 버그가 있었다. 고정 높이를 주면 `object-contain`이 어떤 비율의 사진이든 그 안에서 안전하게 레터박스한다.
...
- **제목 옆 "?" 정보 버튼 → 바텀시트** — 규칙이 복잡해서 화면에 다 못 담는 내용(뱃지 기준, 정산 규칙)은 `<SectionTitle>` 옆에 원형 `?` 버튼(`h-4 w-4 rounded-full border text-sm border-ink-secondary text-ink-secondary`)을 붙이고, 누르면 `BottomSheet`로 항목별(이모지 + 제목 + 설명) 목록을 띄운다. `StatusBoard`의 "시즌 성공 뱃지" 기준 안내가 원형이고, `SettlementGuideButton`(시즌 리포트의 "정산 결과" 옆)이 같은 패턴을 그대로 재사용한다.
